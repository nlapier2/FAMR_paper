...
#SBATCH --array=1-100  # array of jobs. access job id with $SLURM_ARRAY_TASK_ID.
#SBATCH --account=pi-mstephens

. ~/.bash_profile

# default parameter values
tld=results_lineplot_famr_prune0   # top-level directory for results; do not add /
[ ! -d ${tld} ] && mkdir ${tld}
default_other="-i /project2/mstephens/yuxin/ukb-bloodcells/genotypes/ --ld_dir /project2/mstephens/yuxin/ukb-bloodcells/LD/ --num_samples 10000 --num_loci 100 --snps_per_locus 100 --num_confounders 3 --max_causal 5 --phi_gx 0.05 --phi_gy 0.05 --phi_gz 0.2 --phi_zx 0.5 --susieL 30 --methods=famr_gfa --fa_prune_thresh 0.001 --famr_n_iter 30 --final_prune_thresh 0.001 --two_sample"
default_gx=0.2
default_psix=0.1 
default_psiy=0.05
default_psiw=0.0
...
do
        ...
        cmd="Rscript ukbb_run_sim_and_methods.R "${args}" --gamma_zy $this_zy --gamma_gy $default_gy --beta=$default_beta --num_exposures $default_n_expo --famr_prune $default_prune --criterion max"
...
do
        cmd="Rscript ukbb_run_sim_and_methods.R "${args}" --gamma_zy $default_zy --gamma_gy $default_gy --beta=$this_beta --num_exposures $default_n_expo --famr_prune $default_prune --criterion max"
...
do
        cmd="Rscript ukbb_run_sim_and_methods.R "${args}" --gamma_zy 0.0 --gamma_gy $this_gy --beta=$default_beta --num_exposures $default_n_expo --famr_prune $default_prune --criterion max"
...
do
        cmd="Rscript ukbb_run_sim_and_methods.R "${args}" --gamma_zy $default_zy --gamma_gy $default_gy --beta=$default_beta --num_exposures $this_n_expo --famr_prune $default_prune --criterion max"
...
do
        cmd="Rscript ukbb_run_sim_and_methods.R "${args}" --gamma_zy $default_zy --gamma_gy $default_gy --beta=$default_beta --num_exposures $default_n_expo --famr_prune $this_prune --criterion clumped"
